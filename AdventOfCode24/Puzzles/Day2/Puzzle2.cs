...
public class Puzzle2
{
    ...
    public void Solve()
    {
        var reports = PuzzleInput
            .Split("\n", StringSplitOptions.RemoveEmptyEntries)
            .Select(x => x.Split(" ", StringSplitOptions.RemoveEmptyEntries))
            .Select(x => x.Select(int.Parse).ToArray())
            .ToArray();

        var result = reports.Count(report =>
        {
            ...
            var mutations = Enumerable.Range(0, report.Length)
                .Select(x => report.Where((_, index) => index != x).ToArray());

            return mutations.Any(IsReportSafe);
        });

        Console.WriteLine($"We found {result} safe reports");
    }

    public static bool IsReportSafe(int[] report)
    {
        var increasing = report.Order();
        var decreasing = report.OrderDescending();
        var differences = report.Zip(report.Skip(1), (x, y) => Math.Abs(x - y)).ToList();
        var max = differences.Max();
        var min = differences.Min();

        return max <= 3
            && min >= 1
            && (report.SequenceEqual(increasing) || report.SequenceEqual(decreasing));
    }
